    ...
    bool checkForError(DcpPdu &msg)
    {
        ...
        if (error == DcpError::NONE)
        {
            ...
            case DcpPduType::STC_run:
            {
                ...
                if (opMode != DcpOpMode::NRT && (runPDU.getStartTime() > 0 && runPDU.getStartTime() < time_since_epoch))
                {
#if defined(DEBUG) || defined(LOGGING)
            ...
            case DcpPduType::CFG_steps:
            {
                ...
                if (outputAssignment.find(dataId) != outputAssignment.end())
                {
                    ...
                    for (auto &e : outputAssignment[dataId])
                    {
                        ...
                        {
#if defined(DEBUG) || defined(LOGGING)
                            Log(INVALID_STEPS, setSteps.getSteps(), vr, output.fixedSteps ? std::to_string(output.defaultSteps) : "between " + std::to_string(*output.minSteps) + " and " + std::to_string(*output.maxSteps));
#endif
                            error = DcpError::INVALID_STEPS;
                ...
                if (steps.count(outputConfig.getDataId()) >= 1)
                {
                                                            ...
                                                            steps[outputConfig.getDataId()]))
                    {
                        ...
                        Log(INVALID_STEPS, steps[outputConfig.getDataId()],
                            outputConfig.getSourceVr(), output.fixedSteps ? std::to_string(output.defaultSteps) : "between " + std::to_string(*output.minSteps) + " and " + std::to_string(*output.maxSteps));
#endif
                        if (error == DcpError::NONE)
                        {
                            error = DcpError::INVALID_STEPS;

...
public:
    SlaveOne() : stdLog(std::cout)
    {
        ...
        data_out_file.open("data_out_msd1.csv");
    }
    ...
    void configure()
    {
        ...
        Fe = manager->getInput<float64_t *>(Fe_vr);
        x = manager->getOutput<float64_t *>(x_vr);
        v = manager->getOutput<float64_t *>(v_vr);

        data_out_file << "time,x1,v1,Fe" << std::endl;
    }
    ...
    void doStep(uint64_t steps)
    {
        ...
        {
            *x = *x + (*v) * h;
            *v = *v + (h / param_m) * (*Fe - param_c * (*x) - param_d * (*v));
            simulationTime += h;
        }
        data_out_file << simulationTime << ",";
        data_out_file << *x << ",";
        data_out_file << *v << ",";
        data_out_file << *Fe << std::endl;

        // manager->Log(SIM_LOG, simulationTime, currentStep, *x, *v);
    ...
    SlaveDescription_t getSlaveDescription()
    {
        ...
        caus_x->Float64->start = std::make_shared<std::vector<float64_t>>();
        caus_x->Float64->start->push_back(1.0);
        ...
        caus_v->Float64->start->push_back(0.0);
        slaveDescription.Variables.push_back(make_Variable_output("v", v_vr, caus_v));
        std::shared_ptr<CommonCausality_t> caus_Fe = make_CommonCausality_ptr<float64_t>();
        caus_Fe->Float64->start = std::make_shared<std::vector<float64_t>>();
        caus_Fe->Float64->start->push_back(0);
        slaveDescription.Variables.push_back(make_Variable_input("Fe", Fe_vr, caus_Fe));
...
private:
    ...
    float64_t *x;
    const uint32_t x_vr = 1;
    float64_t *v;
    const uint32_t v_vr = 2;
    float64_t *Fe;
    const uint32_t Fe_vr = 3;
    float64_t param_c = 1.0;
    float64_t param_d = 1.0;
    float64_t param_m = 1.0;
